...
class Param:
    ...
    out_path = '/home/shenling/inpainting/dsu_paris_l2_new_0.005_0.007/test_94999/'
    net_file_name = 'netG_94999.pickle'        
    load_path = out_path + net_file_name
...
class TestData(object):
    ...
    def __getitem__(self, idx):
        ...
        image = image.transpose((2, 0, 1))
        image /= 256.0
        sample = {'image': torch.FloatTensor(image), 'id': torch.Tensor([image_id])}
        return sample
...
def main():
    ...
    netG.load_state_dict(torch.load(Param.load_path))
    netG.eval()

    # trainset = ParisData('paris.csv', RandCrop())
    #trainset = TestData('imagenet_test.csv')
    #trainset = TestData('test.csv')
    trainset = TestData('paris_gt.csv')
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=Param.batch_size, shuffle=False, num_workers=4, drop_last=False)
    ...
    #save_image_plus(out_image.cpu().numpy(), Param.out_path + 'test_image_real_{}.jpg'.format(epoch))            
    #epoch += 1
    ...
    for batch in real_data:
        mse = ((fake_data - real_data)**2).mean()
        # if mse == 0:
        #     return 100

        Mean1 = (fake_data - real_data).abs().mean()
        avg_Mean1 += Mean1

        Mean2 = ((fake_data - real_data)**2).mean()
        avg_Mean2 += Mean2

        Pixel_max = 1.0
        psnr = 20 * math.log10(Pixel_max /math.sqrt(mse.item()))
        avg_psnr += psnr
        

    print(avg_Mean1, avg_Mean2, avg_psnr)
    print("Mean_L1: {:.4f}".format(avg_Mean1 / len(real_data)))
    print("Mean_L2: {:.4f}".format(avg_Mean2 / len(real_data)))
    print("PSNR: {:.4f} dB".format(avg_psnr / len(real_data)))
